...
<?php echo $this->Form->create('PageEntity');?>
  ...
  <?php 
    ...
    echo $this->Form->input('url_filter', array('options' => $urlFilters, 'div' => 'input select medium filter', 'after' => $m['url_filter']));
    echo $this->Form->input('url_filter', array('id' => 'PageEntityUrlFilter2', 'type' => 'textarea', 'disabled' => 'disabled', 'div' => 'input text medium filter hidden', 'after' => $m['url_filter']));
    //    echo $this->Form->input("text_filter", array('value' => $textFilterTemplate, 'after' => $m['text_filter']));
    echo $this->Form->input("text_filter", array('options' => $textFilters, 'div' => 'input select medium filter', 'after' => $m['text_filter']));
