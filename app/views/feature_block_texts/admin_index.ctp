<?php echo $this->element('page_entities/subnav') ?>

<div class="featureBlockTexts index">
	<h2><?php __('Feature Block Texts');?></h2>
	<table cellpadding="0" cellspacing="0">
	<tr>
			<th><?php echo $this->Paginator->sort('id');?></th>
			<th><?php echo $this->Paginator->sort('domain');?></th>
			<th><?php echo $this->Paginator->sort('name');?></th>
			<th><?php echo $this->Paginator->sort('description');?></th>
			<th><?php echo $this->Paginator->sort('features');?></th>
			<th><?php echo $this->Paginator->sort('feature_block_text_id');?></th>
			<th><?php echo $this->Paginator->sort('extract_feature_id');?></th>
			<th class="actions"><?php __('Actions');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($featureBlockTexts as $featureBlockText):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $featureBlockText['FeatureBlockText']['id']; ?>&nbsp;</td>
		<td><?php echo $featureBlockText['FeatureBlockText']['domain']; ?>&nbsp;</td>
		<td><?php echo $featureBlockText['FeatureBlockText']['name']; ?>&nbsp;</td>
		<td><?php echo $featureBlockText['FeatureBlockText']['description']; ?>&nbsp;</td>
		<td><?php echo $featureBlockText['FeatureBlockText']['features']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($featureBlockText['FeatureBlockText']['name'], array('controller' => 'feature_block_texts', 'action' => 'view', $featureBlockText['FeatureBlockText']['id'])); ?>
		</td>
		<td>
			<?php echo $this->Html->link($featureBlockText['ExtractFeature']['domain'], array('controller' => 'extract_features', 'action' => 'view', $featureBlockText['ExtractFeature']['id'])); ?>
		</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View', true), array('action' => 'view', $featureBlockText['FeatureBlockText']['id'])); ?>
			<?php echo $this->Html->link(__('Edit', true), array('action' => 'edit', $featureBlockText['FeatureBlockText']['id'])); ?>
			<?php echo $this->Html->link(__('Delete', true), array('action' => 'delete', $featureBlockText['FeatureBlockText']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $featureBlockText['FeatureBlockText']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
	'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
	));
	?>	</p>

	<div class="paging">
		<?php echo $this->Paginator->prev('<< ' . __('previous', true), array(), null, array('class'=>'disabled'));?>
	 | 	<?php echo $this->Paginator->numbers();?>
 |
		<?php echo $this->Paginator->next(__('next', true) . ' >>', array(), null, array('class' => 'disabled'));?>
	</div>
</div>
<div class="actions">
	<h3><?php __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Feature Block Text', true), array('action' => 'add')); ?></li>
		<li><?php echo $this->Html->link(__('List Feature Block Texts', true), array('controller' => 'feature_block_texts', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Feature Block Text', true), array('controller' => 'feature_block_texts', 'action' => 'add')); ?> </li>
		<li><?php echo $this->Html->link(__('List Extract Features', true), array('controller' => 'extract_features', 'action' => 'index')); ?> </li>
		<li><?php echo $this->Html->link(__('New Extract Feature', true), array('controller' => 'extract_features', 'action' => 'add')); ?> </li>
	</ul>
</div>