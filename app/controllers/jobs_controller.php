...
class JobsController extends AppController {
    ...
    $nutchJobs = $this->NutchJob->find('all', 
    		array('NutchJob.user_id' => $this->currentUser['id'], 'limit' => 5, 'order' => 'NutchJob.id DESC'));

    $this->loadModel("ScentJob");
    $this->ScentJob->recursive = -1;
    $scentJobs = $this->ScentJob->find('all', 
    		array('ScentJob.user_id' => $this->currentUser['id'], 'limit' => 5, 'order' => 'ScentJob.id DESC'));
    ...
    $sparkJobs = $this->SparkJob->find('all', 
    		array('SparkJob.user_id' => $this->currentUser['id'], 'limit' => 5, 'order' => 'SparkJob.id DESC'));
