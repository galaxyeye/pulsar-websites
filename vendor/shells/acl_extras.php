 ...
 **/
	function aco_update() {
		...
		$this->out('controllers');
		// look at each controller in src/controllers
 ...
 **/
	function aco_sync() {
		$this->out('Danger! Be sure you want to do this, if so, please modify the file.');
 ...
 */
	function help() {
		$head  = __("Usage: cakephp acl_extras <command>", true) . "\n";
		$head .= "-----------------------------------------------\n";
		$head .= __("Commands:", true) . "\n\n";

		$commands = array(
			'update' => "\tcakephp acl_extras aco_update\n" .
			...
			'sync' =>	"\tcakephp acl_extras aco_sync\n" . 
			...
			'verify' => "\tcakephp acl_extras verify \$type\n" .
						"\t\t" . __('Verify the tree structure of either your Aco or Aro Trees', true),
			
			'recover' => "\tcakephp acl_extras recover \$type\n" .
