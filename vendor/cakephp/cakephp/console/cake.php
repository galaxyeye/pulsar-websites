 ...
 */
	function _initEnvironment() {
		...
		if (!$this->__bootstrap()) {
			...
			$this->stderr("\tMake sure " . DS . 'cakephp' . DS . 'src exists in ' . CAKE_CORE_INCLUDE_PATH);
			$this->_stop();
		}
		...
		if (!isset($this->args[0]) || !isset($this->params['working'])) {
			...
			$this->stderr('Please make sure that ' . DIRECTORY_SEPARATOR . 'cakephp' . DIRECTORY_SEPARATOR . 'console is in your system path,');
 ...
 */
	function __buildPaths() {
		...
		if (!class_exists('Folder')) {
			require LIBS . DS . 'folder.php';
		}
		$plugins = App::objects('plugin', null, false);
		foreach ((array)$plugins as $plugin) {
			$pluginPath = App::pluginPath($plugin);
			$path = $pluginPath . 'vendor' . DS . 'shells' . DS;
			if (file_exists($path)) {
				$paths[] = $path;
			}
		}

		$vendorPaths = array_values(App::path('vendor'));
 ...
 */
	function _getShell($plugin = null) {
		foreach ($this->shellPaths as $path) {
			...
			$pluginShellPath =  DS . $plugin . DS . 'vendor' . DS . 'shells' . DS;
 ...
 */
	function parseParams($params) {
		...
		if (!empty($params['working']) && (!isset($this->args[0]) || isset($this->args[0]) && $this->args[0]{0} !== '.')) {
			if (empty($this->params['src']) && $params['working'] != $params['root']) {
				$params['root'] = dirname($params['working']);
				$params['src'] = basename($params['working']);
			} else {
				$params['root'] = $params['working'];
			}
		}

		if ($params['src'][0] == '/' || preg_match('/([a-z])(:)/i', $params['src'], $matches)) {
			$params['root'] = dirname($params['src']);
		} elseif (strpos($params['src'], '/')) {
			$params['root'] .= '/' . dirname($params['src']);
		}

		$params['src'] = basename($params['src']);
		$params['working'] = rtrim($params['root'], '/') . '/' . $params['src'];
 ...
 */
	function help() {
		...
		$this->stdout(" -src: ". $this->params['src']);
		...
		$this->stdout("to change your path use the '-src' param.");
		$this->stdout("Example: -src relative/path/to/myapp or -src /absolute/path/to/myapp");
		...
		foreach ($this->shellPaths as $path) {
			...
			} elseif (preg_match('@([^\\\/]*)[\\\/]vendor[\\\/]@', $path, $matches)) {
				$type = $matches[1];
			} elseif (strpos($path, CAKE_CORE_INCLUDE_PATH . DS . 'cakephp') === 0) {
				$type = 'CORE';
			} else {
				$type = 'src';
			}
		...
		$this->stdout("\nTo run a command, type 'cakephp shell_name [args]'");
		$this->stdout("To get help on a specific command, type 'cakephp shell_name help'");
	}
