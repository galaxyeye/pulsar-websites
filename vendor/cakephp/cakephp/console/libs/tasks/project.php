 ...
 * @package       cakephp
 * @subpackage    cakephp.cakephp.console.bake
 ...
 *
 * @package       cakephp
 * @subpackage    cakephp.cakephp.console.src.tasks
 */
 ...
 * finds the src directory in it. Then it calls bake() with that information.
 *
 ...
 */
	function execute($project = null) {
		...
		if ($project) {
			$this->Dispatch->parseParams(array('-src', $project));
		...
		while (!$project) {
			$prompt = __("What is the full path for this src including the src directory name?\n Example:", true);
...
/**
 * Enables Configure::read('Routing.prefixes') in /src/config/core.php
 *
 ...
 */
	function getPrefix() {
		...
		if ($this->interactive) {
			$this->hr();
			$this->out('You need to enable Configure::write(\'Routing.prefixes\',array(\'admin\')) in /src/config/core.php to use prefix routing.');
			...
			if ($this->cakeAdmin($admin) !== true) {
				$this->out(__('Unable to write to /src/config/core.php.', true));
				$this->out('You need to enable Configure::write(\'Routing.prefixes\',array(\'admin\')) in /src/config/core.php to use prefix routing.');
				$this->_stop();
			}
			return $admin . '_';
 ...
 */
	function help() {
		$this->hr();
		$this->out("Usage: cakephp bake project <arg1>");
		$this->hr();
		$this->out('Commands:');
		$this->out();
		$this->out("project <name>");
		$this->out("\tbakes src directory structure.");
