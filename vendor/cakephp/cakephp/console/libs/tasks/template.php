 ...
 * @package       cakephp
 * @subpackage    cakephp.console.src.tasks
...
/**
 * Find the paths to all the installed shell themes in the src.
 *
 * Bake themes are directories not named `skel` inside a `vendor/shells/templates` path.
 *
 ...
 */
	function _findThemes() {
		...
		$separator = DS === '/' ? '/' : '\\\\';
		$core = preg_replace('#src' . $separator . '$#', '', $core);
		$paths[] = $core;
		...
		foreach ($plugins as $plugin) {
			$paths[] = $this->_pluginPath($plugin) . 'vendor' . DS . 'shells' . DS;
		}
