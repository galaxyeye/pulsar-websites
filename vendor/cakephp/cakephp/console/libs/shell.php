...
/**
 * Holds the DATABASE_CONFIG object for the src. Null if database.php could not be found,
 * or the src does not exist.
 *
 * @var DATABASE_CONFIG
 * @access public
 */
 ...
 */
	function _checkArgs($expectedNum, $command = null) {
		...
		if (count($this->args) < $expectedNum) {
			...
			$message[] = "Please type `cakephp {$this->shell} help` for help";
 ...
 */
	function help() {
		if ($this->command != null) {
			...
			$this->err("For usage, try `cakephp {$this->shell} help`.", 2);
		} else {
			$this->Dispatch->help();
		}
	}
