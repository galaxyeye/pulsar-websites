 ...
 */
	var $_templates = array(
		...
		'html' => array(
			'trace' => '<pre class="cakephp-debug trace"><b>Trace</b> <p>{:trace}</p></pre>',
			'context' => '<pre class="cakephp-debug context"><b>Context</b> <p>{:context}</p></pre>'
		),
		'txt' => array(
 ...
 */
	function __construct() {
		...
		$e .= '<div id="{:id}-trace" class="cakephp-stack-trace" style="display: none;">';
		$e .= '{:links}{:info}</div>';
		$e .= '</pre>';
		$this->_templates['js']['error'] = $e;

		$t = '<div id="{:id}-trace" class="cakephp-stack-trace" style="display: none;">';
		...
		$this->_templates['js']['context'] = '<pre id="{:id}-context" class="cakephp-context" ';
		$this->_templates['js']['context'] .= 'style="display: none;">{:context}</pre>';

		$this->_templates['js']['code'] = '<div id="{:id}-code" class="cakephp-code-dump" ';
		$this->_templates['js']['code'] .= 'style="display: none;"><pre>{:code}</pre></div>';

		$e = '<pre class="cakephp-debug"><b>{:error}</b> ({:code}) : {:description} ';
		$e .= '[<b>{:path}</b>, line <b>{:line}]</b></pre>';
		$this->_templates['html']['error'] = $e;

		$this->_templates['html']['context'] = '<pre class="cakephp-debug context"><b>Context</b> ';
 ...
 */
	function trimPath($path) {
		...
		if (strpos($path, APP_SRC) === 0) {
			return str_replace(APP_SRC, 'APP' . DS, $path);
		} elseif (strpos($path, CAKE_CORE_INCLUDE_PATH) === 0) {
			return str_replace(CAKE_CORE_INCLUDE_PATH, 'CORE', $path);
		} elseif (strpos($path, ROOT) === 0) {
			return str_replace(ROOT, 'ROOT', $path);
		}
		$corePaths = App::core('cakephp');

		foreach ($corePaths as $corePath) {
			if (strpos($path, $corePath) === 0) {
				return str_replace($corePath, 'CORE' .DS . 'cakephp' .DS, $path);
			}
		}
		return $path;
	}
 ...
 */
	public static function checkSecurityKeys() {
		...
		if (Configure::read('Security.cipherSeed') === '76859309657453542496749683645') {
			trigger_error(__('Please change the value of \'Security.cipherSeed\' in src/config/core.php to a numeric (digits only) seed value specific to your application', true), E_USER_NOTICE);
		}
	}
 ...
 *
 * @param CakeObject $debugger A reference to the Debugger object
 * @access public
 * @static
 * @link http://book.cakephp.org/view/1191/Using-the-Debugger-Class
 */
