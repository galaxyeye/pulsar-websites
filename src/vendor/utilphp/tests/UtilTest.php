...
class UtilityPHPTest extends PHPUnit_Framework_TestCase
{
    ...
    {
        // Regular tests
        $this->assertEquals( '/src/admin/users?user=5', util::add_query_arg( 'user', 5, '/src/admin/users' ) );
        $this->assertEquals( '/src/admin/users?user=5', util::add_query_arg( array( 'user' => 5 ), '/src/admin/users' ) );
        $this->assertEquals( '/src/admin/users?user=5', util::add_query_arg( array( 'user' => 5 ), null, '/src/admin/users' ) );
        $this->assertEquals( '/src/admin/users?action=edit&user=5', util::add_query_arg( 'user', 5, '/src/admin/users?action=edit' ) );
        $this->assertEquals( '/src/admin/users?action=edit&user=5', util::add_query_arg( array( 'user' => 5 ), '/src/admin/users?action=edit' ) );
        $this->assertEquals( '/src/admin/users?action=edit&tab=personal&user=5', util::add_query_arg( 'user', 5, '/src/admin/users?action=edit&tab=personal' ) );
        $this->assertEquals( '/src/admin/users?action=edit&tab=personal&user=5', util::add_query_arg( array( 'user' => 5 ), '/src/admin/users?action=edit&tab=personal' ) );

        // Ensure strips false.
        $this->assertEquals('/index.php', util::add_query_arg('debug', false, '/index.php'));

        // With a URL fragment
        $this->assertEquals( '/src/admin/users?user=5#test', util::add_query_arg( 'user', 5, '/src/admin/users#test' ) );
        ...
        // Url encoding test
        $this->assertEquals( '/src/admin/users?param=containsa%26sym', util::add_query_arg( 'param', 'containsa&sym', '/src/admin/users' ) );

        // If not provided, grab the URI from the server.
        $_SERVER['REQUEST_URI'] = '/src/admin/users';
        $this->assertEquals( '/src/admin/users?user=6', util::add_query_arg( array( 'user' => 6 ) ) );
        $this->assertEquals( '/src/admin/users?user=7', util::add_query_arg( 'user', 7 ) );
    }

    public function test_remove_query_arg()
    {
        $this->assertEquals( '/src/admin/users', util::remove_query_arg( 'user', '/src/admin/users?user=5' ) );
        $this->assertEquals( '/src/admin/users?action=edit', util::remove_query_arg( 'user', '/src/admin/users?action=edit&user=5' ) );
        $this->assertEquals( '/src/admin/users?user=5', util::remove_query_arg( array( 'tab', 'action' ), '/src/admin/users?action=edit&tab=personal&user=5' ) );
    }
