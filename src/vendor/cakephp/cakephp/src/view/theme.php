 ...
 * @package       cakephp
 * @subpackage    cakephp.cakephp.src.view
 ...
 * Theme view class
 *
 * Allows the creation of multiple themes to be used in an src. Theme views are regular view files
 * that can provide unique HTML and static assets.  If theme views are not found for the current view
 * the default src view files will be used. You can set `$this->theme` and `$this->view = 'Theme'` 
 * in your Controller to use the ThemeView.
 *
 * Example of theme path with `$this->theme = 'super_hot';` Would be `src/views/themed/super_hot/posts`
 *
 * @package       cakephp
 * @subpackage    cakephp.cakephp.src.view
 */
		...
		if (!empty($this->theme)) {
			...
			for ($i = 0; $i < $count; $i++) {
				if (strpos($paths[$i], DS . 'plugins' . DS) === false
					&& strpos($paths[$i], DS . 'src' . DS . 'view') === false) {
						if ($plugin) {
