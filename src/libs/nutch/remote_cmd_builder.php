...
class RemoteCmdBuilder extends \Object
{
    ...
    {
        $crawl = $this->crawl['Crawl'];
        assert(!empty($crawl['solrCollection']));

        $command = $this->createCommand($crawl['crawlId'], JobType::INDEX, $crawl['batchId'], $crawl['configId']);
        // $command->getJobConfig()->setArgument("solr.server.url", $crawl['solrServerUrl']);
        $command->getJobConfig()->setArgument("solr.collection", $crawl['solrCollection']);
