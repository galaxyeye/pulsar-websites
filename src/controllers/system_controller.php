...
class SystemController extends AppController {
	...
	public function testSoap() {
		...
		$result = $client->call ( 'ReceiveMsg', array (
				'in0' => $requestBody 
		), '', '', false, true );
	...
	public function fixOliviaPrizeBug($id = null) {
		...
		$count = $this->GmAward->find ( 'count', array (
				'conditions' => array (
						'user_id' => $inviter_id,
						'prize_id' => $prize_id,
						'status' => 'CREATED' 
		...
		if ($count == 2) {
			...
			// Update status
			$this->Game->query ( "UPDATE `gm_awards` AS `GmAward` SET `GmAward`.`status` = 'CANCELED'" . " WHERE `user_id`=$inviter_id AND `status`='CREATED' AND `prize_id`=" . OLIVIA_PRIZE_ID_JIDUKA );
			$this->Game->query ( "UPDATE `aggregation_prize_winners` AS `Winner` SET `Winner`.`status` = 'CANCELED'" . " WHERE `user_id`=$inviter_id AND `status`='CREATED' AND `prize_id`=" . OLIVIA_PRIZE_ID_JIDUKA );
			
			$sql = "INSERT INTO `gm_awards`(`gm_ad_id`, `gm_ad_package_id`, `prize_id`, `user_id`)" . " VALUES($gm_ad_id, $gm_ad_package_id, $prize_id, $inviter_id)";
			$this->Game->query ( $sql );
		} else {
	...
	public function admin_seo() {
		...
		if (empty ( $this->data )) {
			...
			$sql = "UPDATE `prizes` SET `page_keywords`=$page_keywords, `page_description`=$page_description" . " WHERE `is_clone`=1 AND `prototype_id`=$prize_id";
	...
	public function disableAdWithoutPrize() {
		$sql = "update `gm_ads` as `GmAd` set `GmAd`.`status`='CREATED' where `GmAd`.`id` not in " . " (select `Prize`.`gm_ad_id` from `prizes` AS `Prize`)";
