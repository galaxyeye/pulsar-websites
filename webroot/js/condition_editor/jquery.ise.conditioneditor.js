...
(function($) {
	...
	$.widget("ise.conditioneditor", $.ise.treeintable, {
		...
		_makeEquationObject : function(currentItem, leftValue, operator, rightValue) {
		...
		fillColumnNode:function(columnNode, values,trNode, treetableArrayItem, treetableArray,store,repeaterItem, attributes, attributeIndex){
			...
			var imagePath ="/img/condition_editor/condition.gif";
			//var bool =(dataItem.type == "LogicalExpression" || dataItem.type == "UnaryExpression")
			if (values == "!"){
				imagePath= "/img/condition_editor/not.gif";
				textValue="NOT";
			}else if (values == "&&"){
				imagePath= "/img/condition_editor/and.gif";
				textValue="AND";
			}else if (values == "||"){
				imagePath= "/img/condition_editor/or.gif";
				textValue="OR";
			}
